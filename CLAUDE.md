...
- **Tools** (`js/tools/`): All extend `BaseTool` which provides `stampBrush()`. Tools receive document-space coordinates from `CanvasView`. `stampBrush` calls `layer.ensureRect()` then `layer.setPixelAutoExtend()` to auto-grow layers. Selection tools (`RectSelector`, `EllipseSelector`) support resize handles, Shift-to-add, Alt-to-subtract. `FreeTransformTool` lifts pixels to a floating selection and applies scale/rotate/translate via inverse matrix mapping with nearest-neighbor sampling. `FillTool` does flood fill respecting selection boundaries.
